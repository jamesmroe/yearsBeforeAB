...
#          using MRIs at a given distance from AB+. Random intercepts and slopes are generated
#          to emulate subject-specific variability around a nonlinear age trajectory.
...
yearsBeforeAB = 1               # choose distance from AB+ in years
...
plotCheck(check1, visit_age, "usable ADNI")
plotCheck(check2, visit_age, "usable BACS")
plotCheck(check3, visit_age, "usable LCBC")
plotCheck(ucheck1, visit_age, "unusable ADNI")
plotCheck(ucheck2, visit_age, "unusable BACS")
...
converterlist = unique(c(converters_allMRI_negfirst_ADNINC$subject_id, converters_allMRI_negfirst_BACS$subject_id, converters_allMRI_negfirst_LCBC$subject_id))
nonconverterlist = unique(c(nonconverters_allMRI_ADNINC$subject_id, nonconverters_allMRI_BACS$subject_id, nonconverters_allMRI_LCBC$subject_id))
...
for (i in 1:length(ROIs)){
  ...
  if (simulate) {
    ...
    # Random intercepts and slopes are generated
    # to emulate subject-specific variability around a nonlinear age trajectory
    
    DF$subject_id = as.character(DF$subject_id)
    
    # random intercept per subject
    rand_intercept = rnorm(length(unique(DF$subject_id)), 0, 0.05)
    names(rand_intercept) = unique(DF$subject_id)
    
    # random slope per subject
    rand_slope = rnorm(length(unique(DF$subject_id)), 0, 0.002)
    names(rand_slope) = unique(DF$subject_id)
    
    DF$brainvar =
      DF$brainvar +
      rand_intercept[DF$subject_id] +
      rand_slope[DF$subject_id] * DF$visit_age
    
    ggplot(DF, aes(y=brainvar, x = visit_age, col = scanStrength)) + geom_line(data=DF,aes(x=visit_age,brainvar,group=subject_id),color="grey",alpha=0.6, size=0.5)
