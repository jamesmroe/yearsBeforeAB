...
#================ INPUTS =================# uncomment if running on HPC from reproduce.sh
...
#=========================================# uncomment if running on HPC from reproduce.sh
...
#=========================================# comment out if running on HPC from reproduce.sh
# # input tests
simulate = TRUE
analysnum = 1
...
if (simulate) {
  ...
  #---load simulated data
  load(file.path(b, "reproduce/data/simulated_data.Rda"))
  
  subs = unique(simulated_data$subject_id)
  subs_converters = unique(simulated_data$subject_id[simulated_data$convertgroup == 1])
  subs_converters = subs_converters[!is.na(subs_converters)]
  
  # 80 simulated converters
  set.seed(123)
  sampled_times = sample(sampled_times_from_ab[sampled_times_from_ab > 7], 80)
  
  # add sampled time as first MRI timepoint time-from-Ab
  DF = simulated_data
  DF$diff_mriAge_ABpos = NA
  DF %<>% group_by(subject_id) %>% mutate(TimeBsl = visit_age - min(visit_age))
  DF$imageLink = paste0(DF$subject_id, "----", DF$visit_age)
  
  # compute time-from-Ab from other ages 
  for (i in 1:length(subs_converters)) {
    
    print(paste(i, "/", length(subs_converters)))
    sub = subs_converters[i]
    min_age = min(DF$visit_age[DF$subject_id == sub], na.rm = TRUE)
    ages = DF$visit_age[DF$subject_id == sub]
    years_to_abpos = sampled_times[i] - (ages - min_age)
    DF$diff_mriAge_ABpos[DF$subject_id == sub] = years_to_abpos
    # print(
    # DF %>% filter(subject_id == sub) %>% select(subject_id, contains("age"), diff_mriAge_ABpos, TimeBsl)
    # )
  ...
  female_subs = sample(subs, size = floor(n_subs / 2))
  DF$subject_sex = ifelse(DF$subject_id %in% female_subs, "Female", "Male")
...
if (simulate) {
  ...
  # get simulated converters per cohort
  converters_allMRI_negfirst_ADNINC = DF %>% filter(convertgroup == 1 & cohort == "ADNINC")
  converters_allMRI_negfirst_BACS = DF %>% filter(convertgroup == 1 & cohort == "BACS")
  converters_allMRI_negfirst_LCBC = DF %>% filter(convertgroup == 1 & cohort == "LCBC")
  
  converters_allMRI_negfirst_ADNINC$diff_mriAge_predABpos = converters_allMRI_negfirst_ADNINC$diff_mriAge_ABpos - 1
  converters_allMRI_negfirst_BACS$diff_mriAge_predABpos = converters_allMRI_negfirst_BACS$diff_mriAge_ABpos - 1
  converters_allMRI_negfirst_LCBC$diff_mriAge_predABpos = converters_allMRI_negfirst_LCBC$diff_mriAge_ABpos - 1
  
  converters_allMRI_negfirst_ADNINC$ageAtFirstABpos = converters_allMRI_negfirst_ADNINC$visit_age + converters_allMRI_negfirst_ADNINC$diff_mriAge_ABpos
  converters_allMRI_negfirst_BACS$ageAtFirstABpos = converters_allMRI_negfirst_BACS$visit_age + converters_allMRI_negfirst_BACS$diff_mriAge_ABpos
  converters_allMRI_negfirst_LCBC$ageAtFirstABpos = converters_allMRI_negfirst_LCBC$visit_age + converters_allMRI_negfirst_LCBC$diff_mriAge_ABpos
  
  # get simulated AB- per cohort
  nonconverters_allMRI_ADNINC = DF %>% filter(convertgroup == 0 & cohort == "ADNINC")
  nonconverters_allMRI_BACS = DF %>% filter(convertgroup == 0 & cohort == "BACS")
  nonconverters_allMRI_LCBC = DF %>% filter(convertgroup == 0 & cohort == "LCBC")
}
...
ls(pattern = "\\.all") # variables ending with *.all are always those that are loaded in
print("----------------------")
...
for (ii in 1:length(rois)) {
  ...
  if (!simulate) {
    ...
    # simulate mean centiloids
    U.all.tmp = U.all
    set.seed(123)
    n = nrow(U.all.tmp)
    x = rnorm(n, mean = 25, sd = 15)
    U.all.tmp$meanCentiloids = x
  }
  ...
  if (simulate) {
    neverABlist = unique(DF.all$subject_id[DF.all$convertgroup == 0])
    U.all.tmp$convgroup = U.all.tmp$convertgroup
  }
