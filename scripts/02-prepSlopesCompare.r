...
loadPackages = function() {
  packages = c("here", "tidyverse","magrittr","gamm4","itsadug","numDeriv","gratia","mgcv","viridis","wesanderson","asbio","broom","cowplot","data.table","stringi","ggseg","effectsize")
...
rois = corthickrois
# subvolrois = ROIs.all[grepl("vol\\.", ROIs.all)]
# rois = subvolrois
...
# add in new data for review models
# link APOE ----
# LCBC
gID = fread(file.path(b, "reproduce/data/gID_MOAS_match.tsv"))
APOE_LCBC = rbind(fread(file.path(b, "reproduce/data/LCBC_b1_APOE_count.txt")),
                  fread(file.path(b, "reproduce/data/LCBC_b2_APOE_count.txt")))
APOE_LCBC %<>% mutate(carrier = ifelse(E4>=1,1,0))
APOE_LCBC$IID = NULL
names(APOE_LCBC)[1] = "Genetic_ID"

# check all are in gID file
sum(APOE_LCBC$Genetic_ID %in% gID$Genetic_ID) == length(APOE_LCBC$Genetic_ID)
gID = gID %>% select(Genetic_ID, CrossProject_ID, trusted, genetic_duplicate, european) %>% 
  merge(., APOE_LCBC) %>% rename(subject_id = CrossProject_ID)


NEWDAT_LCBC = fread(file.path(b, "/reproduce/data/df.edu.uio.csv")) %>% rename(subject_id = rid)
NEWDAT_BACS = fread(file.path(b, "/reproduce/data/df_bacs_subjinfo.csv")) %>% rename(subject_id = lblid)
NEWDAT_ADNI = fread(file.path(b, "/reproduce/data/ADNIMERGE.csv")) %>% select(PTID, APOE4, PTEDUCAT) %>% rename(subject_id = PTID)

NEWDAT_BACS$carrier = ifelse(NEWDAT_BACS$apoe1 == "E4" | NEWDAT_BACS$apoe2 == "E4", 1, 0)
NEWDAT_BACS$nE4 = ifelse(NEWDAT_BACS$apoe1 == "E4" & NEWDAT_BACS$apoe2 == "E4",
                         2,
                         ifelse(NEWDAT_BACS$apoe1 == "E4" | NEWDAT_BACS$apoe2 == "E4",
                                1,
                                0))
gID$nE4 = gID$E4
NEWDAT_ADNI$nE4 = NEWDAT_ADNI$APOE4
NEWDAT_ADNI$carrier = ifelse(NEWDAT_ADNI$APOE4 > 0, 1, 0)
GEN = rbind(gID %>% select(subject_id, carrier, nE4),
            NEWDAT_BACS %>% select(subject_id, carrier, nE4),
            NEWDAT_ADNI %>% select(subject_id, carrier, nE4)
) %>% mutate(genetic_data = 1) %>% distinct()


# link EDU ----
# harmonize edu
EDU = rbind(
  NEWDAT_LCBC %>% select(subject_id, edu),
  NEWDAT_BACS %>% select(subject_id, edu = `education-yrs`),
  NEWDAT_ADNI %>% select(subject_id, edu = PTEDUCAT)
) %>% distinct()

U.all.edu = left_join(U.all, EDU)
U.all.gen = left_join(U.all.edu, GEN)


# link WMH ----
# white matter hyperintensities
# note that DF.all includes only the ususable MRI observations for each cutoff
WMH = DF.all %>% group_by(subject_id) %>% mutate(meanWHM = mean(vol.WM.hypointensities)) %>% ungroup()
U.all.gen = left_join(U.all.gen, WMH %>% select(subject_id, meanWHM) %>% distinct())
...
for (ii in 1:length(rois)) {
  ...
  if (ii == 1) {
    ...
    loopVar("effectlist", 7, 'l')
    loopVar("effectlist_cent", 7, 'l')
    loopVar("cor", 3, 'v')
    tmpp = tmpp_cent = c()
    p1 = pAll1 = list()
    ...
    # store post-review models
    modlistrev = modlistrev_cent = effectlistrev = effectlistrev_cent = list()
    modlistmainrev = modlistmainrev_cent = effectlistmainrev = effectlistmainrev_cent = list()
    modlistreducedrev = modlistreducedrev_cent = effectlistreducedrev = effectlistreducedrev_cent = list()
    modlistintsexrev = modlistintsexrev_cent = effectlistintsexrev = effectlistintsexrev_cent = list()
    modlistcohortagerev = modlistcohortagerev_cent = effectlistcohortagerev = effectlistcohortagerev_cent = list()
  }
  ...
  # WMH slope
  U.all.tmp$wmhSlope = rSlopeMat.all[,ROIs.all == "vol.WM.hypointensities"]
  ...
  U.all.gen = left_join(U.all.tmp,
                    U.all.gen %>% select(subject_id, edu, carrier, nE4, genetic_data)
  )
  
  
  
  # POST-REVIEW MODELS - apoe, edu and wmh slope corrected models ------
  U.all.gen %<>% filter(!is.na(carrier),
                        !is.na(edu),
                        !is.na(wmhSlope))
  
  print("running models with extra covariates -------------------------------------------")
  modrev = lm(scale(brainvarSlope) ~ (convgroup) + subject_sex + cohort + meanAge + nTimepoints + intervalFirstlast + carrier + scale(edu) + scale(wmhSlope), data = U.all.gen)
  modrev_cent = lm(scale(brainvarSlope) ~ (convgroup) + meanCentiloids + subject_sex + cohort + meanAge + nTimepoints + intervalFirstlast + carrier + scale(edu) + scale(wmhSlope), data = U.all.gen)
  
  # main models with reduced post-review sample
  modmainrev = lm(scale(brainvarSlope) ~ (convgroup) + subject_sex + cohort + meanAge + nTimepoints + intervalFirstlast, data = U.all.gen)
  modmainrev_cent = lm(scale(brainvarSlope) ~ (convgroup) + meanCentiloids + subject_sex + cohort + meanAge + nTimepoints + intervalFirstlast, data = U.all.gen)
  
  modreducedrev = lm(scale(brainvarSlope) ~ (convgroup) + subject_sex + cohort + meanAge + nTimepoints + intervalFirstlast + carrier + scale(edu), data = U.all.gen)
  modreducedrev_cent = lm(scale(brainvarSlope) ~ (convgroup) + meanCentiloids + subject_sex + cohort + meanAge + nTimepoints + intervalFirstlast + carrier + scale(edu), data = U.all.gen)
  nrow(U.all.tmp); nobs(modrev)
  sum(U.all.gen$genetic_data == 1, na.rm = T)
  
  
  # review sex*cohort interaction
  modz = lm(scale(brainvarSlope) ~ (convgroup) + subject_sex + cohort + meanAge + nTimepoints + intervalFirstlast, data = U.all.tmp)
  modz_cent = lm(scale(brainvarSlope) ~ (convgroup) + meanCentiloids + subject_sex + cohort + meanAge + nTimepoints + intervalFirstlast, data = U.all.tmp)
  modintsexrev = update(modz, . ~ . + convgroup*subject_sex)
  modintsexrev_cent = update(modz_cent, . ~ . + convgroup*subject_sex)
  
  # rev age*cohort interaction
  modrevcohortage = update(modz, . ~ . + cohort*visit_age)
  modrevcohortage_cent = update(modz_cent, . ~ . + cohort*visit_age)
  
  
  modlistrev[[ii]] = tidy(modrev)
  modlistrev_cent[[ii]] = tidy(modrev_cent)
  modlistmainrev[[ii]] = tidy(modmainrev)
  modlistmainrev_cent[[ii]] = tidy(modmainrev_cent)
  modlistreducedrev[[ii]] = tidy(modreducedrev)
  modlistreducedrev_cent[[ii]] = tidy(modreducedrev_cent)
  modlistintsexrev[[ii]] = tidy(modintsexrev)
  modlistintsexrev_cent[[ii]] = tidy(modintsexrev_cent)
  modlistcohortagerev[[ii]] = tidy(modrevcohortage)
  modlistcohortagerev_cent[[ii]] = tidy(modrevcohortage_cent)
  
  
  # effect sizes -----
  effectlist1[[ii]] = getCohens(mod)
  effectlist1_cent[[ii]] = getCohens(mod_cent)
  
  
  effectlistrev[[ii]] = getCohens(modrev)
  effectlistrev_cent[[ii]] = getCohens(modrev_cent)
  
  
  # post-review models
  effectlistmainrev[[ii]] = getCohens(modmainrev)
  effectlistmainrev_cent[[ii]] = getCohens(modmainrev_cent)
  
  effectlistreducedrev[[ii]] = getCohens(modreducedrev)
  effectlistreducedrev_cent[[ii]] = getCohens(modreducedrev_cent)

  effectlistintsexrev[[ii]] = getCohens(update(mod, . ~ . + convgroup*subject_sex))
  effectlistintsexrev_cent[[ii]] = getCohens(update(mod_cent, . ~ . + convgroup*subject_sex))
  
  effectlistcohortagerev[[ii]] = getCohens(update(mod, . ~ . + cohort*visit_age))
  effectlistcohortagerev_cent[[ii]] = getCohens(update(mod_cent, . ~ . + cohort*visit_age))
  ...
  if (savefigs) {
    ...
    filename = paste0(plotdir, "/pconv_cortrois-",ii,"-sig", sigflag,"-",rois[ii],"_scale", scaleRes, ".pdf")
    print(paste("saving", filename))
    print("-----------------------")
    ggsave(plot = pAll1[[ii]],
           filename = filename,
           width=9, height=9, units="cm", dpi=600, device=cairo_pdf)
  }
  ...
  # effect sizes -----
  effectlist2[[ii]] = getCohens(modYslopenoage)
  effectlist2_cent[[ii]] = getCohens(modYslopenoage_cent)
  
  effectlist3[[ii]] = getCohens(modYintonly)
  effectlist3_cent[[ii]] = getCohens(modYintonly_cent)
  
  effectlist4[[ii]] = getCohens(modYslopeoys)
  effectlist4_cent[[ii]] = getCohens(modYslopeoys_cent)
  
  effectlist5[[ii]] = getCohens(modYslointooys)
  effectlist5_cent[[ii]] = getCohens(modYslointooys_cent)
  
  effectlist6[[ii]] = getCohens(modYmanchange)
  effectlist6_cent[[ii]] = getCohens(modYmanchange_cent)
  
  effectlist7[[ii]] = getCohens(modYmanchangenoout)
  effectlist7_cent[[ii]] = getCohens(modYmanchangenoout_cent)
  ...
  if (savefigs) {
    ...
    filename_Yintonly = paste0(plotdir_Yintonly, "/pconvYintonly_cortrois-",ii,"-sig", sigflag_Yintonly,"-",roi,"_scale", scaleRes, ".pdf")
    filename_Yintonlycent = paste0(plotdir_Yintonly, "/pconvYintonlycent_cortrois-",ii,"-sig", sigflag_Yintonlycent,"-",roi,"_scale", scaleRes, ".pdf")
    
    filename_Ymanchange = paste0(plotdir_Ymanchange, "/pconvYmanchange_cortrois-",ii,"-sig", sigflag_Ymanchange,"-",roi,"_scale", scaleRes, ".pdf")
    filename_Ymanchangecent = paste0(plotdir_Ymanchange, "/pconvYmanchangecent_cortrois-",ii,"-sig", sigflag_Ymanchangecent,"-",roi,"_scale", scaleRes, ".pdf")
    
    filename_Ymanchangenoout = paste0(plotdir_Ymanchangenoout, "/pconvYmanchangenoout", outlierThresh, "_cortrois-",ii,"-sig", sigflag_Ymanchangenoout,"-",roi,"_scale", scaleRes, ".pdf")
    filename_Ymanchangenooutcent = paste0(plotdir_Ymanchangenoout, "/pconvYmanchangenoout", outlierThresh, "cent_cortrois-",ii,"-sig", sigflag_Ymanchangenooutcent,"-",roi,"_scale", scaleRes, ".pdf")
    
    filename_Yslopoys = paste0(plotdir_Yslopoys, "/pconvYslopoys", outlierThresh, "_cortrois-",ii,"-sig", sigflag_Yslopoys,"-",roi,"_scale", scaleRes, ".pdf")
    filename_Yslopoyscent = paste0(plotdir_Yslopoys, "/pconvYslopoys", outlierThresh, "cent_cortrois-",ii,"-sig", sigflag_Yslopoyscent,"-",roi,"_scale", scaleRes, ".pdf")
    
    filename_Yslointooys = paste0(plotdir_Yslopoys, "/pconvYslointooys", outlierThresh, "_cortrois-",ii,"-sig", sigflag_Yslointooys,"-",roi,"_scale", scaleRes, ".pdf")
    filename_Yslointooyscent = paste0(plotdir_Yslopoys, "/pconvYslointooys", outlierThresh, "cent_cortrois-",ii,"-sig", sigflag_Yslointooyscent,"-",roi,"_scale", scaleRes, ".pdf")
    
    
    filename_maincent = paste0(plotdir_maincent, "/pconvcent_cortrois-",ii,"-sig", sigflag_maincent,"-",roi,"_scale", scaleRes, ".pdf")
...
Z = scaleRes
cortmap = modlist_to_df(modlist1, Z, effectlist1)
cortmap_cent = modlist_to_df(modlist1_cent, Z, effectlist1_cent)

cortmapnoage = modlist_to_df(modlist2, Z, effectlist2)
cortmapnoage_cent = modlist_to_df(modlist2_cent, Z, effectlist2_cent)

cortmapYintonly = modlist_to_df(modlist3, Z, effectlist3)
cortmapYintonly_cent = modlist_to_df(modlist3_cent, Z, effectlist3_cent)

cortmapYslopeoys = modlist_to_df(modlist4, Z, effectlist4)
cortmapYslopeoys_cent = modlist_to_df(modlist4_cent, Z, effectlist4_cent)

cortmapYslointooys = modlist_to_df(modlist5, Z, effectlist5)
cortmapYslointooys_cent = modlist_to_df(modlist5_cent, Z, effectlist5_cent)

cortmapYmanchange = modlist_to_df(modlist6, Z, effectlist6)
cortmapYmanchange_cent = modlist_to_df(modlist6_cent, Z, effectlist6_cent)

cortmapYmanchangenoout = modlist_to_df(modlist7, Z, effectlist7)
cortmapYmanchangenoout_cent = modlist_to_df(modlist7_cent, Z, effectlist7_cent)


# post-review models
cortmaprev = modlist_to_df(modlistrev, Z, effectlistrev)
cortmaprev_cent = modlist_to_df(modlistrev_cent, Z, effectlistrev_cent)
cortmapmainrev = modlist_to_df(modlistmainrev, Z, effectlistmainrev)
cortmapmainrev_cent = modlist_to_df(modlistmainrev_cent, Z, effectlistmainrev_cent)
cortmapreducedrev = modlist_to_df(modlistreducedrev, Z, effectlistreducedrev)
cortmapreducedrev_cent = modlist_to_df(modlistreducedrev_cent, Z, effectlistreducedrev_cent)
cortmaprevallcov = data.frame(rois = rep(rois, each = nrow(tidy(modrev))), bind_rows(modlistrev), bind_rows(effectlistrev))
cortmaprevallcov_cent = data.frame(rois = rep(rois, each = nrow(tidy(modrev_cent))), bind_rows(modlistrev_cent), bind_rows(effectlistrev_cent))
cortmapreducedrevallcov = data.frame(rois = rep(rois, each = nrow(tidy(modreducedrev))), bind_rows(modlistreducedrev), bind_rows(effectlistreducedrev))
cortmapreducedrevallcov_cent = data.frame(rois = rep(rois, each = nrow(tidy(modreducedrev_cent))), bind_rows(modlistreducedrev_cent), bind_rows(effectlistreducedrev_cent))
cortmapmainrevallcov = data.frame(rois = rep(rois, each = nrow(tidy(modmainrev))), bind_rows(modlistmainrev), bind_rows(effectlistmainrev))
cortmapmainrevallcov_cent = data.frame(rois = rep(rois, each = nrow(tidy(modmainrev_cent))), bind_rows(modlistmainrev_cent), bind_rows(effectlistmainrev_cent))
cortmaprevintsex = data.frame(rois = rep(rois, each = nrow(tidy(modintsexrev))), bind_rows(modlistintsexrev), bind_rows(effectlistintsexrev))
cortmaprevintsex_cent = data.frame(rois = rep(rois, each = nrow(tidy(modintsexrev_cent))), bind_rows(modlistintsexrev_cent), bind_rows(effectlistintsexrev_cent))
cortmaprevcohortage = data.frame(rois = rep(rois, each = nrow(modlistcohortagerev[[1]])), bind_rows(modlistcohortagerev), bind_rows(effectlistcohortagerev))
cortmaprevcohortage_cent = data.frame(rois = rep(rois, each = nrow(modlistcohortagerev_cent[[1]])), bind_rows(modlistcohortagerev_cent), bind_rows(effectlistcohortagerev_cent))
...
# post-review models
maprev = name_map("_rev", Z)
maprev_cent = name_map("_revcentcor", Z)
maprevallcov = name_map("_revALLCOV", Z)
maprevallcov_cent = name_map("_revALLCOVCENTCOR", Z)
mapreducedrevallcov = name_map("_REDUCEDrevALLCOV", Z)
mapreducedrevallcov_cent = name_map("_REDUCEDrevALLCOVCENTCOR", Z)
mapmainrevallcov = name_map("_MAINrevALLCOV", Z)
mapmainrevallcov_cent = name_map("_MAINrevALLCOVCENTCOR", Z)
mapintsexrevallcov = name_map("_INTSEXrevALLCOV", Z)
mapintsexrevallcov_cent = name_map("_INTSEXrevALLCOVCENTCOR", Z)
mapcohortagerevallcov = name_map("_COHORTAGErevALLCOV", Z)
mapcohortagerevallcov_cent = name_map("_COHORTAGErevALLCOVCENTCOR", Z)
...
if (writeMaps) {
  ...
  # post-review models
  saveMap(cortmaprev, maprev)
  saveMap(cortmaprev_cent, maprev_cent)
  saveMap(cortmaprevallcov, maprevallcov)
  saveMap(cortmaprevallcov_cent, maprevallcov_cent)
  saveMap(cortmapreducedrevallcov, mapreducedrevallcov)
  saveMap(cortmapreducedrevallcov_cent, mapreducedrevallcov_cent)
  saveMap(cortmapmainrevallcov, mapmainrevallcov)
  saveMap(cortmapmainrevallcov_cent, mapmainrevallcov_cent)
  saveMap(cortmaprevintsex, mapintsexrevallcov)
  saveMap(cortmaprevintsex_cent, mapintsexrevallcov_cent)
  saveMap(cortmaprevcohortage, mapcohortagerevallcov)
  saveMap(cortmaprevcohortage_cent, mapcohortagerevallcov_cent)
...
ggSegIt = function(imap, map_text, statistical_test = T) {
  ...
  if (statistical_test) {
    ...
    # cortical map cohensD -----
    (ggEffectmap = newbrainD + theme_classic() + mytheme +
       scale_fill_gradient2(low = "blue", mid = "white", high = "red",midpoint = 0, limits = c(-.4, .4), oob = scales::squish, na.value = "white") +
       common_text +
       ggtitle(label = paste0(map_text, " ", metric, " ",
                              sighits, "sig, ", fdrhits, "rejections")) +
       theme(legend.position = "top")
    ...
    return(list(ggFacmap = ggFacmap, ggStatmap = ggStatmap, ggEffectmap = ggEffectmap))
...
cortmaprev = ggWrangle(cortmaprev)
cortmaprev_cent = ggWrangle(cortmaprev_cent)
...
if (plotMaps) {
  ...
  maplist1$ggFacmap; maplist1$ggStatmap; maplist1$ggEffectmap
  maplist1_cent$ggFacmap; maplist1_cent$ggStatmap; maplist1_cent$ggEffectmap
  
  
  # post-review models
  maplistrev = ggSegIt(cortmaprev, "rev")
  maplistrev_cent = ggSegIt(cortmaprev_cent, "rev centcor")
  maplistrev$ggFacmap; maplistrev$ggStatmap; maplistrev$ggEffectmap
  maplistrev_cent$ggFacmap; maplistrev_cent$ggStatmap; maplistrev_cent$ggEffectmap
...
if (analysnum != 1 && analysnum != 2) {
  quit()
}


# resampling-based robustness check (write resampled files)
